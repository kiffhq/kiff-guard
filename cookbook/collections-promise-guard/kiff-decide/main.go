// Command kiff-decide — collections-promise-guard gate server.
//
// Routes:
//   POST /v1/proposals/decide
//   POST /v1/events/raw
//   POST /seed        seed a case into DELINQUENT
...
func main() {
	...
	log.Printf("kiff-decide listening on %s (domain: collections-cases)", *addr)
...
func (s *server) decide(w http.ResponseWriter, r *http.Request) {
	...
	case err == nil:
		writeJSON(w, http.StatusOK, decideResponse{ProposalID: propID, Outcome: "allowed",
			Message: "contact cleared; borrower can be reached"})
	case errors.Is(err, action.ErrStateNotAllowed):
		writeJSON(w, http.StatusOK, decideResponse{
			...
			Message: fmt.Sprintf("case is %q — a valid promise exists, contact blocked", currentState),
		})
...
func (s *server) ingest(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil || req.CaseID == "" || req.Type == "" {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "case_id and type are required"})
		return
	}
	actorID := req.ActorID
	if actorID == "" {
	...
	ev := caseEvent(req.CaseID, req.Type, actorID, req.Payload)
...
func (s *server) seed(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil || req.CaseID == "" {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "case_id is required"})
		return
	}
	ev := caseEvent(req.CaseID, EventCaseOpened, "system", req.Payload)
	if err := s.rt.IngestEvent(r.Context(), ev); err != nil {
		writeJSON(w, http.StatusBadGateway, map[string]string{"error": err.Error()})
		return
	}
	writeJSON(w, http.StatusCreated, map[string]string{"status": "seeded", "case_id": req.CaseID, "state": StateDelinquent})
}
