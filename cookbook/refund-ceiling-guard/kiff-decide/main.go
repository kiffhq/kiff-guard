// Command kiff-decide is the refund-ceiling-guard's gate: a small HTTP
// service that wraps the KIFF runtime with the order-refunds domain.
//
// Routes:
//   POST /v1/proposals/decide   the gate
//   POST /v1/events/raw         advance state after a real refund
//   POST /seed                  seed an order into PAID
...
func main() {
	...
	log.Printf("kiff-decide listening on %s (domain: order-refunds)", *addr)
...
func (s *server) decide(w http.ResponseWriter, r *http.Request) {
	...
	case err == nil:
		writeJSON(w, http.StatusOK, decideResponse{
			...
			Message: "refund action contract satisfied; safe to execute",
		})
	case errors.Is(err, action.ErrStateNotAllowed):
		writeJSON(w, http.StatusOK, decideResponse{
			ProposalID: propID, Outcome: "blocked", Reasons: []string{"state_not_allowed"},
			Message: fmt.Sprintf("order is in state %q, which does not permit %s", currentState, req.ActionName),
		})
...
func (s *server) ingest(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil || req.OrderID == "" || req.Type == "" {
		writeJSON(w, http.StatusBadRequest, map[string]string{"error": "order_id and type are required"})
		return
	}
	actorID := req.ActorID
	if actorID == "" {
	...
	ev := orderEvent(req.OrderID, req.Type, actorID, req.Payload)
...
func (s *server) seed(w http.ResponseWriter, r *http.Request) {
	...
	ev := orderEvent(req.OrderID, EventOrderPaid, "system", req.Payload)
	if err := s.rt.IngestEvent(r.Context(), ev); err != nil {
		writeJSON(w, http.StatusBadGateway, map[string]string{"error": err.Error()})
		return
	}
	writeJSON(w, http.StatusCreated, map[string]string{"status": "seeded", "order_id": req.OrderID, "state": StatePaid})
}
