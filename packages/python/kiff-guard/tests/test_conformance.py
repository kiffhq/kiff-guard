...
# --- LlamaIndex: middleware (async). Drives the SDK-independent core
#     (run_guard_tool_call) with an async run_tool continuation. ----------
def _drive_llama_index(guard, tool, args, *, will_run):
    import asyncio

    from kiff_guard.adapters.llama_index_core import run_guard_tool_call
    ...
    async def _go():
        return await run_guard_tool_call(guard, tool, args, run_tool)

    try:
        asyncio.run(_go())
    except Hold:
        pass  # enforce withheld -> Hold raised; tool didn't run
    return ran["v"]
